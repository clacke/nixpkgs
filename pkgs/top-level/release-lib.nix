...
rec {
  ...
  inherit (mkPkgsFor null) pkgsFor;


  # More poor man's memoisation
  pkgsForCross = let
    examplesByConfig = listToAttrs (map
      (n: let v = lib.systems.examples.${n}; in nameValuePair v.config v)
      (builtins.attrNames lib.systems.examples));
  in crossSystem: let
    errorMsg = "unsupported crossSystem: ${crossSystem}";
    candidate = examplesByConfig.${crossSystem.config} or errorMsg;
  in if lib.matchAttrs crossSystem candidate
    then candidate
    else throw errorMsg;
